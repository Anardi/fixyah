                       ...
                       <div class="box-body table-responsive no-padding">
                         ...
                         <table id="example1" class="table table-bordered table-striped">
                           ...
                           <tbody>
                              ...
                              <tr>
                                <th scope="row">{{ $no++ }}</th>
                                <td>{{ $key->id_kode_jadwal }}</td>
                                <td>{{ $key->hari }}</td>
                                <td>{{ $key->jam_mulai }}</td>
                                <td>{{ $key->jam_selesai }}</td>                             
                                <td>
                                  <a href="{{ route('editJadwal', $key->id_kode_jadwal) }}" class="btn btn-info btn-sm mx-1" >
                                    <i class="fas fa-edit"></i>
                                  </a>
                                  <form action="{{ route('deleteJadwal', $key->id_kode_jadwal) }}" method="post" class="d-inline">
                                      @method('delete')
                                      @csrf
